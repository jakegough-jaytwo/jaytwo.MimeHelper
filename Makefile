...
build: restore
	dotnet build ./jaytwo.MimeHelper.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.MimeHelper.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.MimeHelper.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.MimeHelper; \
